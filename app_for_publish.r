...
server <- function(input, output,session) {
  ...
  title_change <- reactive({
    input$change
    str_remove_all(as.character(paste(scan(input$datafile$datapath,what="", sep="\n")[-1],sep = "",collapse = ""))," ")
    
  })
  
  #output$class <- renderTable(types)
  #output$classs <- renderTable(types)
  observeEvent(input$change, { output$test <- renderText({ str_remove_all(title_change(),"[[:cntrl:]]")
    
  })
  fasta_name <- scan(input$datafile$datapath,what="", sep="\n")[1]
  output$fasta_nam <- renderText({fasta_name})
  })
  observeEvent(input$update,{output$pI <- renderText(c("pI =",as.character(round(pI(str_remove_all(as.character(paste(scan(input$datafile$datapath,what="", sep="\n")[-1],sep = "",collapse = ""))," ")),3))))})
  
  observeEvent(input$update,{output$mw <- renderText(c("mw = ",as.character(mw(str_remove_all(as.character(paste(scan(input$datafile$datapath,what="", sep="\n")[-1],sep = "",collapse = ""))," ")),monoisotopic = T),"Da"))})
  observeEvent(input$update,{
    summary <- as.data.frame(aaComp(str_remove_all(as.character(paste(scan(input$datafile$datapath,what="", sep="\n")[-1],sep = "",collapse = ""))," ")))
